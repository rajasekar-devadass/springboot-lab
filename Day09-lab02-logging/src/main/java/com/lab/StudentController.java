...
public class StudentController {
    ...
    @PostMapping("/")
    public ResponseEntity<Object> add(@RequestBody Student student, BindingResult bindingResult) {
        logger.trace("student create method invoked");
        student.setId(studentList.size());
        studentList.add(student);
        logger.debug("name: " + student.getName());
        logger.trace("Student object created");
        return new ResponseEntity<>(student, HttpStatus.CREATED);
    }

    @PutMapping("/")
    public ResponseEntity<Student> put(@RequestBody Student inputStudent) {
        logger.trace("put method called");
